...
load_dll_cmd = LoadDllSymbols()
print("'load-dll' command has been loaded. Use: load-dll PATH_TO_DLL")

dll_loaded = False

def auto_load_dll_on_stop(event):
    """Automatically load DLL symbols when stopped after attach"""
    global dll_loaded
    if not dll_loaded:
        try:
            # Check if we're attached to a process
            result = gdb.execute("info proc", to_string=True)
            if "process" in result.lower():
                dll_loaded = True
                print("Auto-loading DLL symbols after attach...")
                load_dll_cmd.invoke("", False)
        except:
            pass  # Not attached yet or some other issue

# Register the stop event handler
try:
    gdb.events.stop.connect(auto_load_dll_on_stop)
    print("Auto-loader registered: DLL symbols will load automatically after attach")
except:
    print("Warning: Could not register auto-loader")
