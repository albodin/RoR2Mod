...
typedef MonoImage*(__cdecl* mono_image_open_from_data_with_name_t)(char* data, uint32_t data_len, int need_copy, void* status, int refonly, const char* name);
typedef MonoImage* (*mono_image_open_from_data_t)(char* data, uint32_t data_len, int need_copy, void* status);
typedef MonoAssembly* (*mono_assembly_load_from_full_t)(MonoImage* image, const char* fname, void* status, int refonly);
